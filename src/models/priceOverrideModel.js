...
function getOverrideForUserProduct(userId, productId) {
  return get(
...
function upsertPriceOverride({ userId, productId, overridePrice }) {
  run(
...
function deletePriceOverride(id) {
  run('DELETE FROM price_overrides WHERE id = ?', [id]);
}
